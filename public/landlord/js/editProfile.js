...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        const response = await fetch('/api/getProfileLandlord');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addRoom() {
        ...
        // Meter ID label and input
        const meterIDLabel = document.createElement('label');
        meterIDLabel.className = 'add-room-label';
        meterIDLabel.innerText = `Meter ID`;

        const meterIDInput = document.createElement('input');
        meterIDInput.className = 'room-input';
        meterIDInput.type = 'text';
        meterIDInput.name = `meterID${roomIndex}`; // Give each input a unique name
        meterIDInput.placeholder = `Enter meter ID`;

        // Container for room, email, and meterID inputs
        ...
        inputContainer.appendChild(meterIDLabel);
        inputContainer.appendChild(meterIDInput);
    ...
    event.preventDefault();
        ...
        inputs.forEach(inputContainer => {
            ...
            const meterIDInput = inputContainer.querySelector('input[name^="meterID"]');
            
            if (roomInput && emailInput && meterIDInput) {
                roomsData.push({
                    room: roomInput.value,
                    email: emailInput.value,
                    meterID: meterIDInput.value
                });
            }
        });
...
document.addEventListener('DOMContentLoaded', (event) => {
    const input = document.getElementById('username','email','dateOfBirth','meterSerialNumber','houseAddress','meterType');
