document.addEventListener('DOMContentLoaded', function () {
    console.log("DOM fully loaded and parsed");

    // Function to extract data from a specific container
    function extractData(container) {
        try {
            const data = {
                room_name: container.querySelector('.room-name').innerText,
                date: container.querySelector('.empty-k-w').innerText,
                reading_time: container.querySelector('.div1').innerText,
                reading_value: container.querySelector('.kw').innerText,
                last_reading_time: container.querySelector('.div2').innerText,
                last_reading_value: container.querySelector('.kw1').innerText,
                last_reading_date: container.querySelector('.div3').innerText,
                percentage: container.querySelector('.percentage-number').innerText,
                price: container.querySelector('.ghs-120').innerText,
                tenantId: container.dataset.tenantId, // Use data attribute for Tenant ID
                landlordId: localStorage.getItem('uniqueId') // Add Landlord ID
            };
            console.log("Data extracted:", data);
            return data;
        } catch (error) {
            console.error("Error extracting data:", error);
            return {};
        }
    }

    // Attach event listeners to CSV and PDF download buttons within each room container
    document.querySelectorAll('.containers').forEach(container => {
        container.querySelector('.csvbtn').addEventListener('click', function () {
            console.log("CSV download initiated");
            const data = extractData(container);
            const csvData = [
                ['Label', 'Value'],
                ['Room Name', data.room_name],
                ['Date', data.date],
                ['Reading Time', data.reading_time],
                ['Reading Value', data.reading_value],
                ['Last Reading Time', data.last_reading_time],
                ['Last Reading Value', data.last_reading_value],
                ['Last Reading Date', data.last_reading_date],
                ['Percentage Consumption', data.percentage],
                ['Price', data.price],
                ['Tenant ID', data.tenantId],
                ['Landlord ID', data.landlordId]
            ];

            let csvContent = "data:text/csv;charset=utf-8,";
            csvData.forEach(rowArray => {
                let row = rowArray.join(",");
                csvContent += row + "\r\n";
            });

            const encodedUri = encodeURI(csvContent);
            const link = document.createElement("a");
            link.setAttribute("href", encodedUri);
            link.setAttribute("download", "data.csv");
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            console.log("CSV download completed");
        });

        container.querySelector('.R').addEventListener('click', function () {
            console.log("PDF download initiated");
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();
            const data = extractData(container);
            if (Object.keys(data).length === 0) {
                console.error("No data to download");
                return;
            }

            // Add background image
            const imageUrl = 'data:image/png;base64,'; // Replace with your base64 image data       

            const imageWidth = 150; // Width of the image in the PDF
            const imageHeight = 150; // Height of the image in the PDF
            const pageWidth = doc.internal.pageSize.getWidth();
            const pageHeight = doc.internal.pageSize.getHeight();
            const imageX = (pageWidth - imageWidth) / 2;
            const imageY = (pageHeight - imageHeight) / 2;

            try {
                doc.addImage(imageUrl, 'PNG', imageX, imageY, imageWidth, imageHeight);
                console.log("Image added to PDF");
            } catch (error) {
                console.error("Error adding image to PDF:", error);
            }

            doc.setFontSize(22);
            doc.setFont('helvetica', 'bold');
            doc.text("Summary Report", 10, 35);

            doc.setFontSize(16);
            doc.setFont('helvetica', 'normal');
            doc.text("WatCal User Consumption", 10, 45);

            const labels = ['Room Name', 'Date', 'Reading Time', 'Reading Value', 'Last Reading Time', 'Last Reading Value', 'Last Reading Date', 'Percentage Consumption', 'Consumption Cost', 'Tenant ID', 'Landlord ID'];
            const values = [data.room_name, data.date, data.reading_time, data.reading_value, data.last_reading_time, data.last_reading_value, data.last_reading_date, data.percentage, data.price, data.tenantId, data.landlordId];

            doc.setFontSize(12);
            labels.forEach((label, index) => {
                doc.setFont('helvetica', 'bold');
                doc.text(`${label}:`, 10, 60 + (index * 10));
                doc.setFont('helvetica', 'normal');
                doc.text(values[index], 65, 60 + (index * 10));
            });

            doc.save('report.pdf');
            console.log("PDF download completed");
        });
    });
});














