...
document.addEventListener('DOMContentLoaded', () => {
  ...
  yesButton.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch(`/api/deleteRoomHistory/${roomNumber}/${month}/${year}`, {
        method: 'DELETE'
      });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchRoomDataForMonth(month, year) {
        try {
            const response = await fetch(`/api/totalReadingCost/${month}/${currentYear}`);
            const roomData = await response.json();

            displayRoomData(roomData, month);
        } catch (error) {
    ...
    function displayRoomData(data, month) {
        ...
        roomDiv.dataset.month = month;  // Ensure `month` is set correctly
        roomDiv.dataset.year = currentYear;    // Ensure `year` is set correctly
